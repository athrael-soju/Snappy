...
Tables~\ref{tab:category_results_8b},~\ref{tab:category_results}, and~\ref{tab:category_results_vbert} present performance variation across document categories for all three models.

\begin{table}[htbp]
\centering
\caption{ColQwen3-8B performance by document category. The 8B model achieves similar overall accuracy to the 4B variant, with minor variations across categories.}
...
\textbf{Category-Specific Performance Gaps.} Table~\ref{tab:model_gap} reveals that the performance gap between ColQwen3-4B and ColModernVBERT varies systematically across document categories.
...
On BBox-DocVQA, we evaluate three ColPali-family models. ColQwen3-8B and ColQwen3-4B achieve nearly identical localization accuracy (59.8\% and 59.7\% hit rate at IoU@0.5, respectively), while ColModernVBERT achieves 45.5\%. All models provide substantial token savings compared to full-page retrieval (51--58\%), consistent with the efficiency gains predicted by Theorem~\ref{thm:context}.

Category analysis across eight arXiv domains reveals systematic performance variation: computer science and electrical engineering documents achieve higher localization accuracy (Mean IoU $\geq$ 0.65) than economics (0.51) and mathematics (0.38) documents, with physics (0.58--0.59) and quantitative biology (0.62) falling in between.
