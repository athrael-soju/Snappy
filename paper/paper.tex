...
\textbf{Architectural Differences.} ColQwen3-4B merges Qwen3-VL-4B (vision encoder) with Qwen3-Embedding-4B (text encoder) \citep{qwen2025qwen3}, producing 320-dimensional multi-vector embeddings with up to 1,280 visual tokens per page. ColModernVBERT combines a 150M-parameter ModernBERT \citep{warner2024modernbert} text encoder with SigLIP2-base-16b \citep{tschannen2025siglip2} (86M parameters), using early fusion where visual patch embeddings are projected into the language model embedding space. The 16$\times$ parameter difference manifests in query embedding discriminability: larger language models produce sharper attention distributions that concentrate relevance on fewer patches, improving localization precision.

\textbf{Training Data Composition.} The models differ substantially in training data:
\begin{itemize}
    \item \textbf{ColQwen3-4B}: Fine-tuned on VDR (Visual Document Retrieval), ViDoRe-ColPali-Training, and VisRAG-Ret-Train \citep{shi2024visrag} (OpenBMB synthetic and in-domain data), emphasizing diverse document layouts and synthetic query-document pairs.
    \item \textbf{ColModernVBERT}: Trained on The Cauldron 2 and Docmatix (21\% OCR/document content) for modality alignment, then contrastively trained on 118k ColPali query-document pairs with 300k text-only pairs from RLHN, using a 2:1 text-to-image ratio.
\end{itemize}

The higher text-to-image ratio in ColModernVBERT's training may dilute visual grounding capabilities, while ColQwen3-4B's emphasis on synthetic document-query pairs provides broader coverage of document layouts encountered in evaluation.
