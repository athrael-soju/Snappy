...
The \emph{smallest} performance gap occurs in mathematics documents ($\Delta = 0.012$ Mean IoU), where both models achieve their lowest absolute performance. This convergence suggests that mathematics documents approach a \emph{fundamental precision ceiling} imposed by patch quantization rather than model capacity. Equations and dense mathematical notation produce small regions that cannot be accurately localized regardless of model quality, consistent with the theoretical bounds in Section~\ref{sec:theory}.
...
\textbf{Implications for Model Selection.} This analysis suggests two distinct regimes:
\begin{enumerate}
    \item \textbf{Model-dependent regime}: For documents with large, well-separated regions (CS, EESS, Q-Bio), larger models provide substantial accuracy gains.
    \item \textbf{Precision-bound regime}: For documents with dense, small regions (math, economics tables), model improvements yield diminishing returns due to fundamental patch quantization limits.
\end{enumerate}

Applications should consider document characteristics when selecting models: ColModernVBERT's 16$\times$ parameter efficiency may be acceptable for mathematics-heavy corpora where both models approach the same precision ceiling, while ColQwen3-4B's accuracy advantage justifies its cost for technical documentation with varied layouts.
...
\textbf{Deployment Readiness by Document Type.} The two-regime analysis (Section~\ref{sec:model_analysis}) provides actionable guidance for enterprise deployment. For document types in the \emph{model-dependent regime} (technical documentation, scientific papers with figures, mixed prose and table content), current models achieve 60--78\% localization accuracy at IoU@0.5, suitable for production RAG systems where retrieved regions provide substantial context reduction with acceptable precision. For document types in the \emph{precision-bound regime} (mathematics-heavy content, dense financial tables), organizations can either accept current accuracy with the understanding that it reflects fundamental patch quantization limits rather than model deficiencies, or defer deployment until higher-resolution patch models (e.g., SigLIP NaFlex variants \citep{tschannen2025siglip2}) mature. This distinction enables principled deployment decisions: the approach is enterprise-ready for the majority of document types, with well-characterized limitations for the remainder.
...
Analysis of model architecture and training data effects reveals two distinct performance regimes. In the \emph{model-dependent regime}, exemplified by computer science and electrical engineering documents, ColQwen3-4B's 16$\times$ larger capacity yields substantial improvements over ColModernVBERT (up to 35 percentage points at IoU@0.5). In the \emph{precision-bound regime}, exemplified by mathematics documents, both models converge to similar performance ($\Delta = 1.6$ pp) as patch quantization limits dominate. This finding has practical implications: smaller, efficient models may suffice for document types where fundamental precision bounds constrain accuracy regardless of model capacity.
