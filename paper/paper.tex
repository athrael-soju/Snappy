...
The ViDoRe benchmark \citep{faysse2024colpali} evaluates visual document retrieval across diverse domains, measuring NDCG@5 for page-level retrieval. ColPali achieves strong performance, but the benchmark, like the model, operates at page granularity; region-level retrieval remains an unexplored mystery.
...
We model the expected improvement in context quality from region-level retrieval. This analysis provides theoretical motivation; empirical validation of downstream LLM answer quality is outside our scope.
...
\textbf{Models.} We evaluate two ColPali-family models: ColQwen3-4B (4B parameters), representing state-of-the-art accuracy, and ColModernVBERT (250M parameters), representing the efficiency frontier.
...
ColPali's patch attention localizes relevant regions without additional training. With ColQwen3-4B, 84\% of retrieved regions overlap ground-truth evidence boxes at IoU $\geq$ 0.25, and 59.7\% at IoU $\geq$ 0.5. ColModernVBERT (250M parameters, 16$\times$ smaller than ColQwen3-4B) achieves lower localization accuracy (45.5\% at IoU@0.5) but higher token efficiency (38\% savings vs OCR, 58\% vs full image), demonstrating a quality-efficiency tradeoff that may suit different deployment scenarios.

\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth]{figures/iou_thresholds_comparison.png}
\caption{Hit rate comparison across IoU thresholds. ColQwen3-4B consistently outperforms ColModernVBERT, with the gap widening at stricter thresholds.}
\label{fig:iou_thresholds}
\end{figure}

\subsection{Category Analysis}

Table~\ref{tab:category_results} and Table~\ref{tab:category_results_vbert} present performance variation across document categories for both models.
...
\begin{table}[htbp]
\centering
\caption{ColModernVBERT performance by document category. The smaller model shows consistent performance degradation across all categories, with the largest gaps in computer science and electrical engineering where ColQwen3-4B excels.}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{figures/token_comparison.png}
\caption{Token usage comparison across methods. Both hybrid approaches achieve substantial reductions compared to full image (4.00M) and all OCR (2.68M) baselines, with ColModernVBERT offering the greatest efficiency at 1.66M tokens.}
\label{fig:tokens}
\end{figure}

The hybrid approach with percentile-50 filtering reduces context tokens by 29\% compared to returning all OCR regions and by 52\% compared to full-page image tokens. These savings directly reduce LLM inference costs and context window usage.

\begin{figure}[H]
\centering
\includegraphics[width=0.65\textwidth]{figures/radar_comparison.png}
\caption{Multi-dimensional comparison of ColQwen3-4B and ColModernVBERT. The larger model dominates on accuracy metrics (IoU thresholds), while the smaller model offers marginally better token savings. Mean IoU is scaled to percentage for visual consistency.}
\label{fig:radar}
\end{figure}
...
The percentile threshold controls the precision-efficiency tradeoff: lower thresholds retain more regions (higher recall), while higher thresholds improve token savings at the cost of potentially filtering relevant content. Table~\ref{tab:threshold_sensitivity} characterizes this tradeoff on a stratified subset (N=405, 25\% of dataset).

\begin{table}[htbp]
\centering
\caption{Threshold selection guide for ColQwen3-4B. Applications can tune the threshold based on whether they prioritize localization accuracy or token efficiency.}
\label{tab:threshold_sensitivity}
\begin{tabular}{lccc}
\toprule
\textbf{Threshold} & \textbf{IoU@0.5} & \textbf{Token Savings} & \textbf{Regions/Page} \\
\midrule
P25 & XX.X\% & XX.X\% & X.X \\
P50 & XX.X\% & XX.X\% & X.X \\
P75 & XX.X\% & XX.X\% & X.X \\
...
The substantial performance gap between ColQwen3-4B and ColModernVBERT (14.2 percentage points at IoU@0.5) invites analysis of how model architecture and training data affect patch-to-region relevance propagation.

\textbf{Architectural Differences.} The models differ fundamentally in attention mechanism and vision encoder design. ColQwen3-4B uses a causal decoder architecture with Qwen3-VL's $\sim$675M parameter vision encoder \citep{qwen2025qwen3}, which employs \emph{DeepStack} multi-level feature fusion, injecting visual tokens from multiple intermediate ViT layers into corresponding decoder layers to preserve fine-grained details. The encoder uses Interleaved M-RoPE (Multimodal Rotary Position Embedding) that uniformly interleaves temporal, horizontal, and vertical positional components, providing balanced frequency allocation for 2D spatial layouts. ColModernVBERT instead combines a 150M-parameter ModernBERT \citep{warner2024modernbert} text encoder with SigLIP2-base-16b \citep{tschannen2025siglip2} ($\sim$100M parameters), using \emph{bidirectional attention} throughout. SigLIP2's vision encoder benefits from captioning-based pretraining, self-distillation, and masked prediction, producing denser feature representations optimized for localization.

\textbf{Attention Mechanism Impact.} The bidirectional versus causal attention distinction substantially affects late interaction quality. Bidirectional attention allows each token embedding to incorporate full context from both directions, producing richer representations for fine-grained MaxSim matching. In controlled experiments, bidirectional encoders demonstrate a +10.6 nDCG@5 advantage over causal decoders at equivalent parameter counts for retrieval tasks. This explains ColModernVBERT's ability to achieve within 0.6 nDCG@5 of models 10$\times$ larger on \emph{page-level} retrieval benchmarks.

\textbf{Patch Attention and Localization.} For \emph{region-level} localization, however, attention \emph{sharpness} matters more than global context. Our relevance propagation mechanism (Section~\ref{sec:relevance}) computes per-patch scores via $\text{score}_{\text{patch}}(j) = \max_i S_{ij}$, then aggregates these to OCR regions. Localization accuracy depends on how tightly the model's attention concentrates on semantically relevant patches versus diffusing across the page. ColQwen3-4B's larger capacity produces sharper attention distributions that better discriminate relevant from irrelevant patches, while DeepStack's multi-scale features preserve both coarse layout structure and fine-grained text details in the patch embeddings. M-RoPE's explicit 2D spatial encoding further helps attention patterns respect spatial boundaries. These factors compound to explain the 14.2 percentage point IoU@0.5 gap: ColQwen3-4B's attention more precisely targets content regions, yielding higher overlap with ground-truth bounding boxes.
...
\textbf{Category-Specific Performance Gaps.} Table~\ref{tab:model_gap} reveals that the performance gap between models varies systematically across document categories.

\begin{table}[htbp]
\centering
\caption{Performance gap analysis between ColQwen3-4B and ColModernVBERT by category. The gap narrows substantially for mathematics documents, suggesting both models approach fundamental precision limits for small-region localization.}
\label{tab:model_gap}
\begin{tabular}{lcccc}
\toprule
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{figures/iou_by_category_box.png}
\caption{IoU distribution by document category. Box plots show median, quartiles, and outliers. Categories ordered by mean IoU (highest to lowest). Mathematics documents show consistently low performance with tight variance, while computer science achieves both higher mean and greater spread.}
...
\textbf{Implications for Model Selection.} This analysis suggests two empirically distinct regimes:
\begin{enumerate}
    \item \textbf{Model-sensitive documents}: For documents with large, well-separated regions (CS, EESS, Q-Bio), larger models provide substantial accuracy gains.
    \item \textbf{Uniformly challenging documents}: For mathematics and dense tabular content, both models underperform similarly, suggesting challenges beyond model capacity alone.
\end{enumerate}

Applications should consider document characteristics when selecting models: ColModernVBERT's 16$\times$ parameter efficiency may be acceptable for mathematics-heavy corpora where both models achieve similar accuracy, while ColQwen3-4B's accuracy advantage justifies its cost for technical documentation with varied layouts.
...
\textbf{Model-Agnostic Scaling Toward Enterprise Viability.} Because our approach operates at inference time by composing outputs from independent late-interaction and OCR components, it automatically benefits from advances in either domain. Critically, the quality of region-level retrieval depends directly on the \emph{spatial precision of patch-level similarity distributions}: how tightly the model's attention concentrates on semantically relevant page regions. Our comparison between ColQwen3-4B and ColModernVBERT demonstrates this relationship: the larger model's more refined patch attention yields substantially higher localization accuracy (59.7\% vs 45.5\% at IoU@0.5), while the smaller model offers greater token efficiency (58\% vs 52\% savings vs full image). As late-interaction visual retrievers continue to develop more discriminative spatial attention through improved pre-training objectives and higher-resolution patch grids, our approach will inherit these gains without architectural modification. This positions the framework for enterprise deployment: organizations can choose models along the accuracy-efficiency frontier based on their requirements, and upgrade their underlying retrieval model as the field advances.

\textbf{Deployment Readiness by Document Type.} The category analysis (Section~\ref{sec:model_analysis}) provides actionable guidance for enterprise deployment. For model-sensitive document types (technical documentation, scientific papers with figures, mixed prose and table content), current models achieve 60--78\% localization accuracy at IoU@0.5, suitable for production RAG systems where retrieved regions provide substantial context reduction with acceptable precision. For uniformly challenging document types (mathematics-heavy content, dense financial tables), both models achieve similar, lower accuracy; organizations can either accept current performance or wait for improved models with better coverage of these document types. This distinction enables principled deployment decisions: the approach is enterprise-ready for the majority of document types, with well-characterized limitations for the remainder.
...
On BBox-DocVQA with ColQwen3-4B, we achieve 59.7\% hit rate at IoU@0.5 and 52\% token savings compared to full-page retrieval, consistent with the efficiency gains predicted by Theorem~\ref{thm:context}.

Category analysis across eight arXiv domains reveals systematic performance variation: computer science and electrical engineering documents achieve higher localization accuracy (Mean IoU $\geq$ 0.65) than economics (0.51) and mathematics (0.38) documents, with physics (0.58) and quantitative biology (0.62) falling in between.

Analysis of model architecture and training data effects reveals two empirically distinct regimes. For model-sensitive document types (computer science, electrical engineering), ColQwen3-4B's 16$\times$ larger capacity yields substantial improvements over ColModernVBERT (up to 35 percentage points at IoU@0.5). For uniformly challenging document types (mathematics), both models converge to similar performance ($\Delta = 1.6$ pp), suggesting challenges that affect both models similarly. This finding has practical implications: smaller, efficient models may suffice for document types where both models achieve comparable accuracy.
