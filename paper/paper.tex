...
\begin{enumerate}
    ...
    \item \textbf{Empirical Validation and Open Implementation.} We evaluate on BBox-DocVQA, demonstrating 62.3\% hit rate at IoU@0.5 with 54\% token savings versus full-page retrieval (Section~\ref{sec:eval}). We release Snappy, a complete open-source system implementing this architecture (Section~\ref{sec:impl}).
\end{enumerate}

Empirical evaluation on BBox-DocVQA validates that region-level retrieval achieves strong spatial grounding, with 62.3\% of retrieved regions achieving IoU $\geq$ 0.5 with ground-truth evidence bounding boxes, while reducing context tokens by over 54\% compared to full-page retrieval. These results confirm that retrieval granularity directly impacts both precision and efficiency.
...
\textbf{Hybrid (ours)} & \textbf{Low (0.2$\times$)} & \textbf{62.3\%} & \textbf{Precise RAG} \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Dataset.} BBox-DocVQA \citep{bboxdocvqa2025} provides question-answer pairs across documents from arXiv categories including computer science (cs), economics (econ), electrical engineering (eess), mathematics (math), physics, quantitative biology (q-bio), quantitative finance (q-fin), and statistics (stat). Each QA pair includes ground-truth bounding boxes marking the evidence region containing the answer. We evaluate on 1,240 samples across eight categories.
...
\begin{table}[htbp]
\centering
\caption{Spatial grounding accuracy on BBox-DocVQA (n=1240). Hit rates indicate the fraction of samples achieving IoU at or above the specified threshold.}
\label{tab:main_results}
\begin{tabular}{lcccc}
\toprule
\textbf{Model} & \textbf{Mean IoU} & \textbf{IoU@0.25} & \textbf{IoU@0.5} & \textbf{IoU@0.7} \\
\midrule
ColQwen3-4B (P50) & 0.597 & 85.8\% & 62.3\% & 41.9\% \\
\bottomrule
\end{tabular}
\end{table}

The results demonstrate that ColPali's patch attention effectively localizes relevant regions without additional training. Over 85\% of retrieved regions have meaningful overlap (IoU $\geq$ 0.25) with ground-truth evidence boxes, and 62.3\% achieve substantial overlap (IoU $\geq$ 0.5).
...
\caption{Token usage comparison. Our hybrid approach reduces context tokens by 30\% vs.\ full OCR and 54\% vs.\ full image tokens.}
\label{fig:tokens}
\end{figure}
...
\midrule
cs & 176 & 0.723 & 94.9\% & 75.6\% & 65.3\% \\
eess & 156 & 0.709 & 96.8\% & 86.5\% & 58.3\% \\
q-bio & 137 & 0.641 & 91.2\% & 67.9\% & 46.0\% \\
physics & 173 & 0.611 & 88.4\% & 66.5\% & 39.9\% \\
stat & 101 & 0.588 & 89.1\% & 60.4\% & 39.6\% \\
q-fin & 176 & 0.556 & 86.4\% & 61.4\% & 32.4\% \\
econ & 174 & 0.539 & 77.6\% & 48.3\% & 36.8\% \\
math & 148 & 0.395 & 62.2\% & 29.7\% & 13.5\% \\
\midrule
\textbf{Overall} & \textbf{1240} & \textbf{0.597} & \textbf{85.8\%} & \textbf{62.3\%} & \textbf{41.9\%} \\
\bottomrule
\end{tabular}
\end{table}

Computer science and electrical engineering documents achieve substantially higher localization accuracy (Mean IoU $>$ 0.7) than economics (0.539) and mathematics (0.395) documents, with physics (0.611) and quantitative biology (0.641) falling in between. This performance gap likely reflects differences in layout complexity: economics papers frequently contain dense tables with many small cells, while mathematics papers feature equations and formulas that may span multiple regions. These smaller regions suffer disproportionately from patch quantization effects, consistent with the theoretical precision bounds in Section~\ref{sec:theory}.

\subsection{Token Efficiency}

Figure~\ref{fig:tokens} visualizes the token savings achieved by our approach. We measure context tokens that would be passed to a downstream LLM. For full image retrieval, we estimate tokens using Claude's approximation: images are resized to fit within $1568 \times 1568$ pixels while maintaining aspect ratio, then token count is computed as $\lfloor (w \times h) / 750 \rfloor$. For OCR-based methods, we tokenize extracted text using tiktoken's \texttt{cl100k\_base} encoding (GPT-4/Claude compatible). Token counts are summed across all 1,240 evaluation samples. Table~\ref{tab:token_savings} presents token savings under different retrieval strategies.
...
\midrule
Full Image (baseline) & 3,068,485 & -- & -- \\
All OCR Regions & 2,006,575 & -- & 34.6\% \\
\textbf{Hybrid (P50, filtered)} & \textbf{1,411,708} & \textbf{29.6\%} & \textbf{54.0\%} \\
\bottomrule
\end{tabular}
\end{table}

The hybrid approach with percentile-50 filtering reduces context tokens by 30\% compared to returning all OCR regions and by 54\% compared to full-page image tokens. These savings directly translate to reduced LLM inference costs, lower latency, and decreased attention dilution in downstream RAG applications.
...
\textbf{Category-Specific Performance.} Empirical evaluation reveals substantial performance variation across document types. Mathematics documents achieve only 29.7\% hit rate at IoU@0.5 compared to 86.5\% for electrical engineering documents. This gap reflects the patch quantization effects predicted by our theoretical analysis: mathematics papers contain equations and dense notation where relevant content spans small regions that are poorly aligned with the fixed patch grid. Applications targeting specific document types should consider this variation when setting performance expectations.
...
Empirical evaluation on BBox-DocVQA validates our approach. Using ColQwen3-4B with percentile-50 thresholding, we achieve 62.3\% hit rate at IoU@0.5, demonstrating that ColPali's patch attention effectively localizes relevant regions without additional training. Token savings of 54\% compared to full-page retrieval confirm the practical efficiency gains predicted by our theoretical analysis (Theorem~\ref{thm:context}).

Category analysis across eight arXiv domains reveals systematic performance variation: computer science and electrical engineering documents achieve higher localization accuracy (Mean IoU $>$ 0.7) than economics (0.54) and mathematics (0.40) documents, with physics and quantitative biology falling in between. This gap aligns with our theoretical precision bounds: smaller regions in dense tables and equations suffer disproportionately from patch quantization effects.
