...
const ConfigUpdate = z
  ...
  .passthrough();
const HardwareSnapshot = z
  .object({
    cpu_cores: z.number().int(),
    cpu_model: z.union([z.string(), z.null()]).optional(),
    cpu_frequency_ghz: z.union([z.number(), z.null()]).optional(),
    total_ram_gb: z.union([z.number(), z.null()]).optional(),
    nvidia_gpu_count: z.number().int(),
    nvidia_gpu_names: z.union([z.array(z.string()), z.null()]).optional(),
    profile: z.string(),
  })
  .passthrough();
const ConfigOptimizationResponse = z
  .object({
    status: z.string(),
    message: z.string(),
    profile: z.string(),
    applied: z.record(z.string()),
    unchanged: z.array(z.string()),
    detection: HardwareSnapshot,
    services_invalidated: z.boolean(),
  })
  .passthrough();
...
const endpoints = makeApi([
  ...
  },
  {
    ...
    alias: "optimize_config_config_optimize_post",
    description: `Optimise configuration for the current host hardware.
...
runtime configuration values. Returns a summary of applied settings.`,
    requestFormat: "json",
    response: ConfigOptimizationResponse,
  },
  {
    method: "post",
    path: "/config/reset",
