...
Drop-in replacement for jobs.py that uses the streaming pipeline.
"""
...
def run_streaming_indexing_job(
    job_id: str,
    paths: List[str],
    ...
    This is a drop-in replacement for run_indexing_job() that uses
    the new streaming pipeline architecture.
    """
    pipeline = None
    ...
    try:
        ...
        pipeline = StreamingPipeline(
            ...
            image_store=qdrant_svc.indexer._pipeline._batch_processor.image_store,
        ...
        completion_msg = (
            ...
            f"from {len(paths)} document(s) (streaming mode)"
        )
...
def run_indexing_job(job_id: str, paths: List[str], filenames: Dict[str, str]) -> None:
    """
    Adaptive job runner that uses streaming or batch pipeline based on config.

    Set USE_STREAMING_PIPELINE=true in config to enable streaming mode.
    """
    use_streaming = getattr(config, "USE_STREAMING_PIPELINE", False)

    if use_streaming:
        logger.info(f"Job {job_id}: Using STREAMING pipeline")
        return run_streaming_indexing_job(job_id, paths, filenames)
    else:
        logger.info(f"Job {job_id}: Using BATCH pipeline")
        # Import and use original batch pipeline
        from .jobs import run_indexing_job as run_batch_job

        return run_batch_job(job_id, paths, filenames)
