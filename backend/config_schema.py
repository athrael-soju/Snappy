...
CONFIG_SCHEMA: Dict[str, Dict[str, Any]] = {
    ...
    "paddle_ocr_vl": {
        ...
        "icon": "scan-text",
        "name": "Visual OCR",
        "description": "PaddleOCR-VL service integration settings",
        "settings": [
            {
                "key": "PADDLE_OCR_VL_ENABLED",
                "type": "bool",
                "default": False,
                "label": "Enable PaddleOCR-VL",
                "ui_type": "boolean",
                "description": "Enable integration with an external PaddleOCR-VL service",
                "help_text": "Toggle to enable the OCR pipeline. When disabled, OCR endpoints will return 503 responses and the backend will not contact the PaddleOCR-VL service.",
            },
            {
                "key": "PADDLE_OCR_VL_URL",
                "type": "str",
                "default": "http://localhost:8100",
                "label": "Service URL",
                "ui_type": "text",
                "description": "Base URL for PaddleOCR-VL service",
                "help_text": "Root endpoint for the PaddleOCR-VL FastAPI service (without the API prefix). Example: http://localhost:8100. The backend automatically appends the API prefix for OCR requests and uses this base URL for health checks.",
            },
            {
                "key": "PADDLE_OCR_VL_API_PREFIX",
                "type": "str",
                "default": "/api/v1",
                "label": "API Prefix",
                "ui_type": "text",
                "description": "Prefix for PaddleOCR-VL API routes",
                "help_text": "Path prefix added to all API requests sent to the PaddleOCR-VL service. Defaults to /api/v1 which matches the standalone service.",
            },
            {
                "key": "PADDLE_OCR_VL_API_TIMEOUT",
                "type": "int",
                "default": 300,
                "label": "API Timeout (seconds)",
                "ui_type": "number",
                "min": 10,
                "max": 600,
                "description": "Request timeout for PaddleOCR-VL API",
                "help_text": "Maximum wait time for OCR extraction requests. Increase if processing large PDFs or running the service on CPU-only hardware as described in the PaddleOCR-VL setup guide.",
            },
            {
                "key": "PADDLE_OCR_VL_ALLOWED_EXTENSIONS",
                "type": "list",
                "default": ".jpg,.jpeg,.png,.bmp,.tiff,.tif,.pdf",
                "label": "Allowed File Extensions",
                "ui_type": "text",
                "description": "Supported file extensions passed to PaddleOCR-VL",
                "help_text": "Comma separated list of lowercase extensions (including the dot) that the backend should accept before proxying the upload to PaddleOCR-VL. Defaults match the reference configuration from the PaddleOCR-VL documentation.",
            },
            {
                "key": "PADDLE_OCR_VL_MAX_FILE_SIZE_MB",
                "type": "int",
                "default": 50,
                ...
                "min": 1,
                "max": 500,
                "description": "Maximum file size allowed for OCR uploads",
                "help_text": "Reject files above this size before forwarding them to PaddleOCR-VL. The default aligns with the service defaults recommended by PaddleOCR-VL for GPU deployments.",
            },
        ],
    },
    "qdrant": {
...
def get_critical_keys() -> set:
    ...
    """
    return {
        "PADDLE_OCR_VL_ENABLED",
        "PADDLE_OCR_VL_URL",
        "PADDLE_OCR_VL_API_PREFIX",
        "PADDLE_OCR_VL_API_TIMEOUT",
        "MUVERA_ENABLED",
        "QDRANT_EMBEDDED",
