...
CONFIG_SCHEMA: Dict[str, Dict[str, Any]] = {
    ...
    "ocr": {
        ...
        "icon": "scan",
        "name": "DeepSeek OCR",
        "description": "Configuration for the DeepSeek OCR service and runtime defaults.",
        "settings": [
            {
                ...
                "ui_type": "boolean",
                "description": "Toggle DeepSeek OCR integration.",
                "help_text": "Enable to proxy OCR requests through the standalone DeepSeek OCR FastAPI service. Disable when the service is unavailable to avoid connection errors.",
            },
            {
                ...
                "label": "Service URL",
                "ui_type": "text",
                "description": "Base URL for the DeepSeek OCR service.",
                "help_text": "HTTP endpoint for the DeepSeek OCR service (for example: http://localhost:8200). Ensure the backend can reach this URL inside Docker environments.",
                "depends_on": {"key": "DEEPSEEK_OCR_ENABLED", "value": True},
                "ui_hidden": True,
            },
            {
                "key": "DEEPSEEK_OCR_TIMEOUT",
                "type": "int",
                "default": 180,
                ...
                "help_text": "Maximum number of seconds to wait for the OCR service to return results. Large images or CPU execution may need higher values (300-600s).",
                "depends_on": {"key": "DEEPSEEK_OCR_ENABLED", "value": True},
                "ui_hidden": True,
            },
            {
                "key": "DEEPSEEK_OCR_DEFAULT_MODE",
                "type": "str",
                "default": "plain_ocr",
                "label": "Default Mode",
                "ui_type": "select",
                ...
                "description": "Default DeepSeek OCR mode.",
                "help_text": "Mode used when clients do not explicitly select one. Refer to the DeepSeek OCR documentation for detailed behaviour of each mode.",
                "depends_on": {"key": "DEEPSEEK_OCR_ENABLED", "value": True},
            },
            {
                "key": "DEEPSEEK_OCR_DEFAULT_PROMPT",
                "type": "str",
                "default": "",
                "label": "Default Prompt",
                "ui_type": "text",
                "description": "Prompt used for freeform OCR mode.",
                "help_text": "Optional prompt that is applied when using the 'freeform' OCR mode and the caller does not supply a prompt. Leave empty for a generic OCR instruction.",
                "depends_on": {"key": "DEEPSEEK_OCR_ENABLED", "value": True},
            },
            {
                "key": "DEEPSEEK_OCR_DEFAULT_GROUNDING",
                "type": "bool",
                "default": False,
                "label": "Grounding Enabled by Default",
                "ui_type": "boolean",
                "description": "Enable grounding boxes even when not required by the mode.",
                "help_text": "When enabled the backend will request grounding boxes whenever a request omits the grounding flag. Some modes (find_ref, layout_map, pii_redact) always enable grounding regardless of this toggle.",
                "depends_on": {"key": "DEEPSEEK_OCR_ENABLED", "value": True},
            },
            {
                "key": "DEEPSEEK_OCR_DEFAULT_INCLUDE_CAPTION",
                "type": "bool",
                "default": False,
                "label": "Include Caption by Default",
                "ui_type": "boolean",
                "description": "Request an additional caption when not specified by the caller.",
                "help_text": "Adds a descriptive caption to OCR output when clients do not explicitly request one. Useful for accessibility workflows but adds processing overhead.",
                "depends_on": {"key": "DEEPSEEK_OCR_ENABLED", "value": True},
            },
            {
                "key": "DEEPSEEK_OCR_DEFAULT_BASE_SIZE",
                "type": "int",
                "default": 1024,
            ...
            {
                "key": "DEEPSEEK_OCR_DEFAULT_IMAGE_SIZE",
                "type": "int",
                "default": 640,
            ...
            {
                "key": "DEEPSEEK_OCR_DEFAULT_CROP_MODE",
                "type": "bool",
                "default": True,
                "label": "Crop Mode Enabled by Default",
                "ui_type": "boolean",
                "description": "Use crop mode for inference unless overridden.",
                "help_text": "Crop mode can improve accuracy on dense documents by focusing on regions of interest. Disable to process the entire page as a single image.",
                "depends_on": {"key": "DEEPSEEK_OCR_ENABLED", "value": True},
            },
            {
                "key": "DEEPSEEK_OCR_DEFAULT_TEST_COMPRESS",
                "type": "bool",
                "default": False,
                "label": "Test Compression by Default",
                "ui_type": "boolean",
                "description": "Enable compression diagnostics when not specified.",
                "help_text": "When enabled the OCR service records compression metrics without saving intermediate files. Primarily useful for benchmarking.",
                "depends_on": {"key": "DEEPSEEK_OCR_ENABLED", "value": True},
            },
            {
                "key": "DEEPSEEK_OCR_DEFAULT_PROFILE",
                "type": "str",
                "default": "gundam",
                "label": "Default Profile",
                "ui_type": "select",
                "options": ["gundam", "tiny", "small", "base", "large"],
                "description": "Preset controlling base/image size and crop behaviour.",
                "help_text": "Profile presets align with the official DeepSeek demo. Choose 'gundam' for balanced quality, 'tiny' for fastest turnaround, or 'large' for maximum fidelity.",
                "depends_on": {"key": "DEEPSEEK_OCR_ENABLED", "value": True},
            },
            {
                "key": "DEEPSEEK_OCR_RETURN_MARKDOWN",
                "type": "bool",
                "default": False,
                "label": "Return Markdown by Default",
                "ui_type": "boolean",
                "description": "Include markdown output (with inline figures) in OCR responses.",
                "help_text": "Enable to request markdown-formatted text from the DeepSeek service for every call. Disable to save bandwidth when only plain text is needed.",
                "depends_on": {"key": "DEEPSEEK_OCR_ENABLED", "value": True},
            },
            {
                "key": "DEEPSEEK_OCR_RETURN_FIGURES",
                "type": "bool",
                "default": False,
                "label": "Return Figure Crops",
                "ui_type": "boolean",
                "description": "Include base64-encoded crops for detected image regions.",
                "help_text": "Provides inline figure crops (labelled 'image') as data URIs. Useful for rich renderers; disable to reduce payload size.",
                "depends_on": {"key": "DEEPSEEK_OCR_ENABLED", "value": True},
            },
            {
                "key": "DEEPSEEK_OCR_ENABLE_FLASH_ATTN",
                "type": "bool",
                "default": True,
                "label": "Enable FlashAttention",
                "ui_type": "boolean",
                "description": "Attempt to load the model with FlashAttention 2 kernels.",
                "help_text": "Requires GPU execution and a compatible flash-attn installation inside the DeepSeek service container. When unavailable the service automatically falls back to eager attention.",
                "depends_on": {"key": "DEEPSEEK_OCR_ENABLED", "value": True},
            },
        ],
    },
