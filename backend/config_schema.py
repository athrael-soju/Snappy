...
CONFIG_SCHEMA: Dict[str, Dict[str, Any]] = {
    ...
    "ocr": {
        ...
        "icon": "scan",
        "name": "OCR Service",
        "description": "PaddleOCR-VL document parsing service configuration",
        "settings": [
            {
                "key": "PADDLE_OCR_ENABLED",
                "type": "bool",
                "default": False,
                "label": "Enable Paddle OCR",
                "ui_type": "boolean",
                "description": "Toggle integration with the PaddleOCR-VL document parsing service.",
                "help_text": "When enabled the backend forwards document parsing requests to the PaddleOCR-VL microservice. Disable to skip OCR processing while keeping other features available.",
            },
            {
                "key": "PADDLE_OCR_URL",
                "type": "str",
                ...
                "label": "Service URL",
                "ui_type": "text",
                "description": "Base URL for the PaddleOCR-VL FastAPI service.",
                "help_text": "The backend connects to this endpoint for OCR requests. Format: http://hostname:port. Ensure the PaddleOCR-VL container is reachable from the backend network.",
                "depends_on": {"key": "PADDLE_OCR_ENABLED", "value": True},
            },
            {
                "key": "PADDLE_OCR_API_PREFIX",
                "type": "str",
                "default": "/api/v1",
                "label": "API Prefix",
                "ui_type": "text",
                "description": "API prefix configured on the PaddleOCR-VL service.",
                "help_text": "Matches the FastAPI router prefix configured by the service. Defaults to /api/v1. Adjust only if the service was configured with a different prefix.",
                "depends_on": {"key": "PADDLE_OCR_ENABLED", "value": True},
            },
            {
                "key": "PADDLE_OCR_TIMEOUT",
                "type": "int",
                "default": 300,
                ...
                "min": 30,
                "max": 900,
                "description": "Timeout for OCR extraction requests.",
                "help_text": "Larger documents take longer to parse. Increase this value if you encounter timeout errors during OCR processing. Values can range between 30 seconds and 15 minutes.",
                "depends_on": {"key": "PADDLE_OCR_ENABLED", "value": True},
            },
            {
                "key": "PADDLE_OCR_MAX_FILE_MB",
                "type": "int",
                "default": 50,
                ...
                "min": 1,
                "max": 200,
                "description": "Maximum upload size forwarded to PaddleOCR-VL.",
                "help_text": "Requests larger than this limit are rejected before hitting the OCR service. Keep aligned with the service configuration. Acceptable range is 1-200 MB; defaults to 50 MB.",
                "depends_on": {"key": "PADDLE_OCR_ENABLED", "value": True},
            },
            {
                "key": "PADDLE_OCR_USE_LAYOUT_DETECTION",
                "type": "bool",
                "default": True,
                "label": "Enable Layout Detection",
                "ui_type": "boolean",
                "description": "Run the document layout detector before recognition.",
                "help_text": "Layout detection improves block segmentation for complex documents but adds latency. Disable for pre-cropped regions.",
                "depends_on": {"key": "PADDLE_OCR_ENABLED", "value": True},
            },
            {
                "key": "PADDLE_OCR_LAYOUT_THRESHOLD",
                "type": "float",
                "default": 0.3,
                ...
                "max": 1.0,
                "step": 0.05,
                "description": "Confidence threshold for layout detection boxes.",
                "help_text": "Higher values produce fewer layout blocks; lower values are more permissive. Valid range is 0-1.",
                "depends_on": {"key": "PADDLE_OCR_USE_LAYOUT_DETECTION", "value": True},
            },
            {
                "key": "PADDLE_OCR_LAYOUT_NMS",
                "type": "float",
                "default": 0.5,
                "label": "Layout NMS IoU",
                "ui_type": "number",
                "min": 0.0,
                "max": 1.0,
                "step": 0.05,
                "description": "Overlap threshold for suppressing duplicate layout boxes.",
                "help_text": "Adjust to balance between merged and distinct layout regions. Valid range is 0-1.",
                "depends_on": {"key": "PADDLE_OCR_USE_LAYOUT_DETECTION", "value": True},
            },
            {
                "key": "PADDLE_OCR_LAYOUT_UNCLIP_RATIO",
                "type": "float",
                "default": 1.5,
                ...
                "max": 3.0,
                "step": 0.1,
                "description": "Expansion ratio applied to detected layout polygons.",
                "help_text": "Higher ratios pad detected regions to capture full content. Increase for dense documents.",
                "depends_on": {"key": "PADDLE_OCR_USE_LAYOUT_DETECTION", "value": True},
            },
            {
                "key": "PADDLE_OCR_LAYOUT_MERGE_MODE",
                "type": "str",
                "default": "union",
                ...
                "options": [
                    ...
                    {"label": "Union overlap", "value": "union"},
                ],
                "description": "Strategy for merging overlapping layout blocks.",
                "help_text": "Match the merge mode supported by PaddleOCR-VL. Use 'none' for raw boxes or 'union' to join overlapping regions.",
                "depends_on": {"key": "PADDLE_OCR_USE_LAYOUT_DETECTION", "value": True},
            },
            {
                ...
                "type": "bool",
                "default": True,
                "label": "Orientation Classification",
                "ui_type": "boolean",
                "description": "Automatically detect and correct document rotation.",
                "help_text": "Enable to improve accuracy for rotated scans. Disable to reduce preprocessing latency when orientation is known.",
                "depends_on": {"key": "PADDLE_OCR_ENABLED", "value": True},
            },
            {
                ...
                "type": "bool",
                "default": False,
                "label": "Perspective Unwarping",
                "ui_type": "boolean",
                "description": "Apply document unwarping prior to recognition.",
                "help_text": "Improves results for camera captures with curvature at the cost of additional GPU/CPU time.",
                "depends_on": {"key": "PADDLE_OCR_ENABLED", "value": True},
            },
            {
                "key": "PADDLE_OCR_USE_CHART_RECOGNITION",
                "type": "bool",
                "default": False,
                "label": "Chart Recognition",
                "ui_type": "boolean",
                "description": "Enable chart parsing in the OCR pipeline.",
                "help_text": "Chart recognition is compute intensive. Enable only when documents contain plots that need extraction.",
                "depends_on": {"key": "PADDLE_OCR_ENABLED", "value": True},
            },
            {
                "key": "PADDLE_OCR_FORMAT_BLOCK_CONTENT",
                "type": "str",
                "default": "markdown",
                "label": "Block Content Format",
                "ui_type": "select",
                "options": [
                ...
                "description": "Preferred serialization format for OCR block content.",
                "help_text": "Controls how PaddleOCR-VL formats each block. Markdown is recommended for rich text rendering.",
                "depends_on": {"key": "PADDLE_OCR_ENABLED", "value": True},
            },
            {
                "key": "PADDLE_OCR_MIN_PIXELS",
                "type": "int",
                "default": 0,
                "label": "Minimum Resize Pixels",
                "ui_type": "number",
                "min": 0,
                "max": 4096,
                "description": "Minimum target size when scaling the shortest side.",
                "help_text": "Set to 0 to keep PaddleOCR-VL defaults. Increase to upscale low-resolution inputs before recognition.",
                "depends_on": {"key": "PADDLE_OCR_ENABLED", "value": True},
            },
            {
                "key": "PADDLE_OCR_MAX_PIXELS",
                "type": "int",
                "default": 2048,
                "label": "Maximum Resize Pixels",
                "ui_type": "number",
                "min": 0,
                "max": 8192,
                "description": "Maximum target size when scaling the longest side.",
                "help_text": "Caps image size fed into the recognizer. Set to 0 to skip clamping.",
                "depends_on": {"key": "PADDLE_OCR_ENABLED", "value": True},
            },
        ],
    },
